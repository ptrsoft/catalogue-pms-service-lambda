...
export const generateStateMachine2 = (stages) => {
	const { addParallelTaskLambda } = use(API);

	const lambdaArn = addParallelTaskLambda.functionArn;
	...
	for (let i = 0; i < stages.length; i++) {
		...
		const stage = {
			Type: "Task",
			Resource:
				"arn:aws:lambda:us-east-1:654654554404:function:workflow-process-lambda:$LATEST",
			Parameters: {
				[`payload.$`]: "$",
		...
		const nextStageName =
			i < stages.length - 1 ? stages[i + 1].name : "end";
		...
		newStepFunction.States[currentStageName + "-complete"] = {
			Type: "Task",
			Resource:
				"arn:aws:lambda:us-east-1:654654554404:function:workflow-complete-stage",
			ResultPath: "$",
			Next: nextStageName,
		};

		if (i === 0) {
			newStepFunction.States[previousStageName].Next =
				currentStageName + "-tasks";
		} else {
			newStepFunction.States[previousStageName + "-complete"].Next =
				currentStageName;
		}
	...
	newStepFunction.States["end"] = {
		Type: "Task",
		Resource:
			"arn:aws:lambda:us-east-1:654654554404:function:workflow-complete-usecase",
		End: true,
	};
...
export const generateStateMachine1 = (stages) => {
	const newStepFunction = {
		Comment: "My test workflow state machine",
		StartAt: "stages",
		States: {
